...
contract TroveManager is LiquityBase, Ownable, CheckContract, ITroveManager {
    ...
    /*
     * L_AUT and L_LUSDDebt track the sums of accumulated liquidation rewards per unit staked. During its lifetime, each stake earns:
     *
     * An AUT gain of ( stake * [L_AUT - L_AUT(0)] )
     * A LUSDDebt increase  of ( stake * [L_LUSDDebt - L_LUSDDebt(0)] )
     *
     * Where L_AUT(0) and L_LUSDDebt(0) are snapshots of L_AUT and L_LUSDDebt for the active Trove taken at the instant the stake was made
     */
    uint public L_AUT;
    ...
    // Object containing the AUT and LUSD snapshots for a given active trove
    ...
    event Redemption(uint _attemptedLUSDAmount, uint _actualLUSDAmount, uint _AUTSent, uint _AUTFee);
    event TroveUpdated(
    ...
    event LTermsUpdated(uint _L_AUT, uint _L_LUSDDebt);
    event TroveSnapshotsUpdated(uint _L_AUT, uint _L_LUSDDebt);
    ...
    function liquidateTroves(uint _n) external override {
        ...
        // Move liquidated AUT and LUSD to the appropriate pools
    ...
    function _redeemCollateralFromTrove(
        ...
        singleRedemption.AUTLot = singleRedemption.LUSDLot.mul(DECIMAL_PRECISION).div(_price);

        // Decrease the debt and collateral of the current Trove according to the LUSD lot and corresponding AUT to send
     ...
     * The redeemer swaps (debt - liquidation reserve) LUSD for (debt - liquidation reserve) worth of AUT, so the LUSD liquidation reserve left corresponds to the remaining debt.
    ...
    function _redeemCloseTrove(
        ...
        // Update Active Pool LUSD, and send AUT to account
    ...
    function redeemCollateral(
        ...
        emit Redemption(_LUSDamount, totals.totalLUSDToRedeem, totals.totalAUTDrawn, totals.AUTFee);

        // Burn the total LUSD that is cancelled with debt, and send the redeemed AUT to msg.sender
    ...
    function getNominalICR(address _borrower) public view override returns (uint) {
        (uint currentAUT, uint currentLUSDDebt) = _getCurrentTroveAmounts(_borrower);

        uint NICR = LiquityMath._computeNominalCR(currentAUT, currentLUSDDebt);
        return NICR;
    }
    ...
    function getCurrentICR(address _borrower, uint _price) public view override returns (uint) {
        ...
        uint ICR = LiquityMath._computeCR(currentAUT, currentLUSDDebt, _price);
        return ICR;
    }
    ...
    function _getCurrentTroveAmounts(address _borrower) internal view returns (uint, uint) {
        ...
        return (currentAUT, currentLUSDDebt);
    }
    ...
    // Update borrower's snapshots of L_AUT and L_LUSDDebt to reflect the current values
    ...
    function _updateTroveRewardSnapshots(address _borrower) internal {
        ...
        emit TroveSnapshotsUpdated(L_AUT, L_LUSDDebt);
    }
    ...
    function getEntireDebtAndColl(
        ...
        view
        override
        returns (uint debt, uint coll, uint pendingLUSDDebtReward, uint pendingAUTReward)
    {
    ...
    function _redistributeDebtAndColl(
         ...
         * error correction, to keep the cumulative error low in the running totals L_AUT and L_LUSDDebt:
         *
        ...
        emit LTermsUpdated(L_AUT, L_LUSDDebt);
    ...
    function _updateBaseRateFromRedemption(
        ...
        /* Convert the drawn AUT back to LUSD at face value rate (1 LUSD:1 USD), in order to get
         * the fraction of total supply that was redeemed at face value. */
        uint redeemedLUSDFraction = _AUTDrawn.mul(_price).div(_totalLUSDSupply);
