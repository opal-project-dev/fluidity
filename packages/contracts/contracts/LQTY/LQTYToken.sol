...
* 
*
*  --- Functionality added specific to the OPLToken ---
* 
* 1) Transfer protection: blacklist of addresses that are invalid recipients (i.e. core Liquity contracts) in external 
* transfer() and transferFrom() calls. The purpose is to protect users from losing tokens by mistakenly sending OPL directly to a Liquity
* core contract, when they should rather call the right function.
*
* 2) sendToOPLStaking(): callable only by Liquity core contracts, which move OPL tokens from user -> OPLStaking contract.
*
...
* -sendToOPLStaking() reverts when the multisig is the sender, blocking the multisig from staking its OPL.
* 
* After one year has passed since deployment of the OPLToken, the restrictions on multisig operations are lifted
* and the multisig has the same rights as any other address.
*/

contract OPLToken is CheckContract, IOPLToken {
    using SafeMath for uint256;

    // --- ERC20 Data ---

    string internal constant _NAME = "OPL";
    string internal constant _SYMBOL = "OPL";
    ...
    function sendToOPLStaking(address _sender, uint256 _amount) external override {
        _requireCallerIsOPLStaking();
        if (_isFirstYear()) {
    ...
    function permit(
        ...
        require(deadline >= now, "OPL: expired deadline");
        ...
        require(recoveredAddress == owner, "OPL: invalid signature");
    ...
    function _requireSenderIsNotMultisig(address _sender) internal view {
        require(_sender != multisigAddress, "OPLToken: sender must not be the multisig");
    }

    function _requireCallerIsNotMultisig() internal view {
        require(!_callerIsMultisig(), "OPLToken: caller must not be the multisig");
    }
