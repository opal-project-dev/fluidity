...
Ether is the collateral for LUSD. The aut price $P_t^e$ follows 
...
> (collateral in aut, debt in LUSD, collateral ratio target, rational inattention, collateral ratio)
...
def liquidate_troves(accounts, contracts, active_accounts, inactive_accounts, price_aut_current, price_LUSD, price_LQTY_current, data, index):
    ...
    liquidation_gain = aut_liquidated * price_aut_current - debt_liquidated * price_LUSD
...
def close_troves(accounts, contracts, active_accounts, inactive_accounts, price_aut_current, price_LUSD, index):
    if len(active_accounts) == 0:
        return [0]
...
def open_troves(accounts, contracts, active_accounts, inactive_accounts, price_aut_current, price_LUSD, index):
...
def redeem_trove(accounts, contracts, i, price_aut_current):
    ...
    [firstRedemptionHint, partialRedemptionHintNICR, truncatedLUSDamount] = contracts.hintHelpers.getRedemptionHints(lusd_balance, price_aut_current, 70)
...
def price_stabilizer(accounts, contracts, active_accounts, inactive_accounts, price_aut_current, price_LUSD, index):
