...
contract("Gas cost tests", async accounts => {
  ...
  it("", async () => {
    ...
    // 10 accts each open Trove with 10 aut, withdraw 100 ONEU
  ...
  it("", async () => {
    ...
    //60 accts open Trove with 1 aut and withdraw 100 ONEU
  ...
  it("", async () => {
    ...
    // 10 accts each open Trove with 15 aut, withdraw 100 ONEU
    await th.openTrove_allAccounts(
      accounts.slice(101, 111),
      contracts,
      dec(15, "ether"),
    ...
    //65 accts open Trove with 1 aut and withdraw 100 ONEU
  ...
  it("", async () => {
    ...
    // 10 accts each open Trove with 10 ether, withdraw 100 ONEU
    await th.openTrove_allAccounts(
      accounts.slice(101, 111),
      contracts,
      dec(1000, "ether"),
    ...
    //50 accts open Trove with 1 aut and withdraw 100 ONEU
    const _50_Defaulters = accounts.slice(1, 51);
    await th.openTrove_allAccounts(_50_Defaulters, contracts, dec(100, "ether"), dec(9500, 18));
  ...
  it("", async () => {
    ...
    await stabilityPool.provideToSP(dec(1, 27), ZERO_ADDRESS, { from: accounts[999] });
    assert.equal(await stabilityPool.getTotalONEUDeposits(), dec(1, 27));
  ...
  it("", async () => {
    ...
    // 10 accts each open Trove with 10 ether, withdraw 2000 ONEU
    await th.openTrove_allAccounts(
      accounts.slice(101, 111),
      contracts,
      dec(200, "ether"),
    ...
    // Account 500 opens with 1 aut and withdraws 2000 ONEU
    await borrowerOperations.openTrove(_100pct, dec(2000, 18), accounts[500], ZERO_ADDRESS, {
      from: accounts[500],
      value: dec(20, "ether")
    });
