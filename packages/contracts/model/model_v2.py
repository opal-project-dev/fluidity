...
class ModelParams:
    def __init__(self):
        ...
        self.lookback = 5 # Lookback parameter for AUT price momentum
...
### Functions
def get_new_momentum(data, params, AUT_price):
    lookback = params.lookback
    if lookback == 0:
        return 0

    AUT_price_past = get_past_AUT_price(data, params)

    new_momentum = (AUT_price - AUT_price_past) /  AUT_price_past
    return new_momentum

def get_past_AUT_price(data, params):
    length = len(data.AUT_price)
    
    AUT_price_past = None
    if (params.lookback > length):
        AUT_price_past = data.AUT_price[0]
    else:
        AUT_price_past = data.AUT_price[length - params.lookback - 1]

    if AUT_price_past == 0:
        return 1
    return AUT_price_past
...
# return the exogenous market demand for holding OPL tokens. Assume constant. Could be a function of:
...
### Various AUT price functions

def constant_AUT_price(last_price):
    return last_price

# AUT price generator is a random walk (normal dist.), with occasional large +ve and -ve jumps
def randomwalk_AUT_price(last_price):
    big_event = 0
...
def linear_increasing_AUT_price(last_price, gradient):
    return last_price + gradient

def oscillating_AUT_price(min, magnitude, i):
    return min + magnitude + magnitude*np.sin(i)

def linear_decreasing_AUT_price(start, gradient, i):
    val = (start - (gradient*i))
    if val <= 0:
        return 0
    return val

def one_over_i_AUT_price(scale, i):
    return scale/i

def quadratic_AUT_price(min, scale, i):
    return min + scale*(i**2)

def sublinear_AUT_price(last_price, steepness, i):
...
for i in range(1, 250):
    last_AUT_price =  data.AUT_price[-1]

    # update exogenous AUT price

    # AUT_price = last_AUT_price
    AUT_price = randomwalk_AUT_price(last_AUT_price)
    # AUT_price = oscillating_AUT_price(500, 100, i)
    # AUT_price = quadratic_AUT_price(500, 10, i)
    # AUT_price = linear_increasing_AUT_price(last_AUT_price, 3)
    # AUT_price = linear_decreasing_AUT_price(800, 1, i)
    # AUT_price = one_over_i_AUT_price(1000, i)
    # AUT_price = sublinear_AUT_price(last_AUT_price, 10, i)
    
    momentum = get_new_momentum(data, params, AUT_price)
    ...
    data.AUT_price.append(AUT_price)
...
ax3.set_title('AUT Price')
plt.ylim(0, 1000)
plt.plot(data.AUT_price)
