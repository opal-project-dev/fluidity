...
contract("TroveManager - in Recovery Mode - back to normal mode in 1 tx", async accounts => {
  ...
  context("Batch liquidations", () => {
    ...
    it("Stability Pool profit matches", async () => {
      ...
      const spEthBefore = await stabilityPool.getAUT();
      const spLusdBefore = await stabilityPool.getTotalONEUDeposits();
      ...
      const spEthAfter = await stabilityPool.getAUT();
      const spLusdAfter = await stabilityPool.getTotalONEUDeposits();
      ...
      const expectedGainInONEU = expectedCollateralLiquidatedA
        .mul(price)
        .div(mv._1e18BN)
        .sub(A_totalDebt);
      const realGainInONEU = spEthAfter
        .sub(spEthBefore)
        .mul(price)
        .div(mv._1e18BN)
      ...
      assert.equal(
        ...
        "Stability Pool AUT doesn’t match"
      );
      assert.equal(
        spLusdBefore.sub(spLusdAfter).toString(),
        A_totalDebt.toString(),
        "Stability Pool ONEU doesn’t match"
      );
      assert.equal(
        realGainInONEU.toString(),
        expectedGainInONEU.toString(),
