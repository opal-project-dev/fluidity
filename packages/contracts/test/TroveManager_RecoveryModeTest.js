...
contract("TroveManager - in Recovery Mode", async accounts => {
  ...
  it("checkRecoveryMode(): Returns true if TCR falls below CCR", async () => {
    ...
    // price drops to 1AUT:150ONEU, reducing TCR below 150%.  setPrice() calls checkTCRAndSetRecoveryMode() internally.
  ...
  it("liquidate(), with ICR < 100%: updates system snapshots correctly", async () => {
    ...
    // --- TEST ---
    // price drops to 1AUT:100ONEU, reducing TCR below 150%, and all Troves below 100% ICR
    await priceFeed.setPrice("100000000000000000000");
  ...
  it("liquidate(), with 100 < ICR < 110%: removes stake and updates totalStakes", async () => {
    // --- SETUP ---
    //  Bob withdraws up to 2000 ONEU of debt, bringing his ICR to 210%
    ...
    // --- TEST ---
    // price drops to 1AUT:100ONEU, reducing TCR to 117%
    await priceFeed.setPrice("100000000000000000000");
  ...
  it("liquidate(), with 100% < ICR < 110%: updates system snapshots correctly", async () => {
    ...
    //  Bob withdraws up to 20000 ONEU of debt, bringing his ICR to 210%
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(150, 16)),
      extraParams: { from: alice }
    });
  ...
  it("liquidate(), with 100% < ICR < 110%: offsets as much debt as possible with the Stability Pool, then redistributes the remainder coll and debt", async () => {
    ...
    // Alice deposits 390ONEU to the Stability Pool
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: alice });
    ...
    // check pool ONEU before liquidation
    const stabilityPoolONEU_Before = (await stabilityPool.getTotalONEUDeposits()).toString();
    assert.equal(stabilityPoolONEU_Before, "390000000000000000000");
    ...
    /* Now, liquidate Bob. Liquidated coll is 21 aut, and liquidated debt is 2000 ONEU.
    
    With 390 ONEU in the StabilityPool, 390 ONEU should be offset with the pool, leaving 0 in the pool.
  
    Stability Pool rewards for alice should be:
    ONEULoss: 390ONEU
    AUTGain: (390 / 2000) * 21*0.995 = 4.074525 aut

    After offsetting 390 ONEU and 4.074525 aut, the remainders - 1610 ONEU and 16.820475 aut - should be redistributed to all active Troves.
   */
    // Liquidate Bob
    await troveManager.liquidate(bob, { from: owner });

    const aliceDeposit = await stabilityPool.getCompoundedONEUDeposit(alice);
    const aliceAUTGain = await stabilityPool.getDepositorAUTGain(alice);
    const aliceExpectedAUTGain = spDeposit.mul(th.applyLiquidationFee(B_coll)).div(B_totalDebt);

    assert.equal(aliceDeposit.toString(), 0);
    assert.equal(aliceAUTGain.toString(), aliceExpectedAUTGain);

    /* Now, check redistribution to active Troves. Remainders of 1610 ONEU and 16.82 aut are distributed.
    ...
    L_ONEUDebt = 1610 / 6 = 268.333 ONEU
    L_AUT = 16.820475 /6 =  2.8034125 aut
    */
    const L_ONEUDebt = (await troveManager.L_ONEUDebt()).toString();
  ...
  it("liquidate(), with ICR > 110%, trove has lowest ICR, and StabilityPool is empty: does nothing", async () => {
    ...
    assert.equal(L_ONEUDebt, "0");
    assert.equal(L_AUT, "0");
  ...
  // --- liquidate(), applied to trove with ICR > 110% that has the lowest ICR, and Stability Pool ONEU is GREATER THAN liquidated debt ---

  it("liquidate(), with 110% < ICR < TCR, and StabilityPool ONEU > debt to liquidate: offsets the trove entirely with the pool", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ONEU of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ONEU of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(240, 16)),
      extraONEUAmount: dec(250, 18),
      extraParams: { from: bob }
    });
    const { collateral: A_coll } = await openTrove({
      ICR: toBN(dec(266, 16)),
    ...
    // Alice deposits ONEU in the Stability Pool
    ...
    /* Check accrued Stability Pool rewards after. Total Pool deposits was 1490 ONEU, Alice sole depositor.
    As liquidated debt (250 ONEU) was completely offset

    Alice's expected compounded deposit: (1490 - 250) = 1240ONEU
    Alice's expected AUT gain:  Bob's liquidated capped coll (minus gas comp), 2.75*0.995 aut
  
    */
    const aliceExpectedDeposit = await stabilityPool.getCompoundedONEUDeposit(alice);
    const aliceExpectedAUTGain = await stabilityPool.getDepositorAUTGain(alice);
  ...
  it("liquidate(), with ICR% = 110 < TCR, and StabilityPool ONEU > debt to liquidate: offsets the trove entirely with the pool, there’s no collateral surplus", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ONEU of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ONEU of debt, resulting in ICR of 220%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(220, 16)),
      extraONEUAmount: dec(250, 18),
      extraParams: { from: bob }
  ...
  it("liquidate(), with  110% < ICR < TCR, and StabilityPool ONEU > debt to liquidate: removes stake and updates totalStakes", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ONEU of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ONEU of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
  ...
  it("liquidate(), with  110% < ICR < TCR, and StabilityPool ONEU > debt to liquidate: updates system snapshots", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ONEU of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ONEU of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
  ...
  it("liquidate(), with 110% < ICR < TCR, and StabilityPool ONEU > debt to liquidate: closes the Trove", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ONEU of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ONEU of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
  ...
  it("liquidate(), with 110% < ICR < TCR, and StabilityPool ONEU > debt to liquidate: can liquidate troves out of order", async () => {
    // taking out 1000 ONEU, CR of 200%
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraParams: { from: alice }
    });
    ...
    await openTrove({
      ICR: toBN(dec(200, 16)),
      extraONEUAmount: totalLiquidatedDebt,
  ...
  ONEU is LESS THAN the liquidated debt: a non fullfilled liquidation --- */

  it("liquidate(), with ICR > 110%, and StabilityPool ONEU < liquidated debt: Trove remains active", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ONEU of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ONEU of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({
      ICR: toBN(dec(266, 16)),
      extraONEUAmount: dec(1500, 18),
      extraParams: { from: alice }
    });
    const { collateral: B_coll } = await openTrove({
      ICR: toBN(dec(240, 16)),
    ...
    // Alice deposits 1490 ONEU in the Stability Pool
    ...
    /* Since the pool only contains 100 ONEU, and Bob's pre-liquidation debt was 250 ONEU,
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool ONEU < liquidated debt: Trove remains in TroveOwners array", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ONEU of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ONEU of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({
    ...
    // Alice deposits 100 ONEU in the Stability Pool
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool ONEU < liquidated debt: nothing happens", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ONEU of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ONEU of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({
    ...
    /*  Since Bob's debt (250 ONEU) is larger than all ONEU in the Stability Pool, Liquidation won’t happen
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool ONEU < liquidated debt: updates system shapshots", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ONEU of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ONEU of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool ONEU < liquidated debt: causes correct Pool offset and AUT gain, and doesn't redistribute to active troves", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ONEU of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ONEU of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({
    ...
    assert.equal(aliceExpectedAUTGain.toString(), "0");
    ...
    const L_ONEUDebt_After = (await troveManager.L_ONEUDebt()).toString();
    const L_AUT_After = (await troveManager.L_AUT()).toString();

    assert.equal(L_ONEUDebt_After, "0");
    assert.equal(L_AUT_After, "0");
  });

  it("liquidate(), with ICR > 110%, and StabilityPool ONEU < liquidated debt: ICR of non liquidated trove does not change", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ONEU of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ONEU of debt, resulting in ICR of 240%. Bob has lowest ICR.
    // Carol withdraws up to debt of 240 ONEU, -> ICR of 250%.
    const { collateral: A_coll } = await openTrove({
      ICR: toBN(dec(266, 16)),
      extraONEUAmount: dec(1500, 18),
      extraParams: { from: alice }
    ...
    //Check SP ONEU has been completely emptied
    assert.equal((await stabilityPool.getTotalONEUDeposits()).toString(), dec(100, 18));
    ...
    await oneuToken.transfer(bob, dec(100, 18), { from: alice });
    ...
    // Alice provides another 50 ONEU to pool
    ...
    //Check SP ONEU has been completely emptied
    assert.equal((await stabilityPool.getTotalONEUDeposits()).toString(), dec(150, 18));
  ...
  it("liquidate() with ICR > 110%, and StabilityPool ONEU < liquidated debt: total liquidated coll and debt is correct", async () => {
    // Whale provides 50 ONEU to the SP
    await openTrove({
      ICR: toBN(dec(300, 16)),
      extraONEUAmount: dec(50, 18),
      extraParams: { from: whale }
    });
  ...
  it("liquidate(): Doesn't liquidate undercollateralized trove if it is the only trove in the system", async () => {
    // Alice creates a single trove with 0.62 AUT and a debt of 62 ONEU, and provides 10 ONEU to SP
  ...
  it("liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    ...
    /*  Since there is 0 ONEU in the stability Pool, A, with ICR >110%, should stay active.
  ...
  it("liquidate(): does not affect the SP deposit or AUT gain when called on an SP depositor's address that has no trove", async () => {
    ...
    //Dennis provides 200 ONEU to SP
    ...
    assert.isAtMost(th.getDifference(dennis_AUTGain_Before, th.applyLiquidationFee(C_coll)), 1000);
  ...
  it("liquidate(), with 110% < ICR < TCR, can claim collateral, re-open, be reedemed and claim again", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ONEU of debt, resulting in ICRs of 266%.
    // Bob withdraws up to 480 ONEU of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(240, 16)),
      extraONEUAmount: dec(480, 18),
      extraParams: { from: bob }
    });
    const { collateral: A_coll } = await openTrove({
      ICR: toBN(dec(266, 16)),
    ...
    // Bob re-opens the trove, price 200, total debt 80 ONEU, ICR = 120% (lowest one)
    // Dennis redeems 30, so Bob has a surplus of (200 * 0.48 - 30) / 200 = 0.33 AUT
  ...
  it("liquidate(), with 110% < ICR < TCR, can claim collateral, after another claim from a redemption", async () => {
    // --- SETUP ---
    // Bob withdraws up to 90 ONEU of debt, resulting in ICR of 222%
    const { collateral: B_coll, netDebt: B_netDebt } = await openTrove({
      ICR: toBN(dec(222, 16)),
      extraONEUAmount: dec(90, 18),
      extraParams: { from: bob }
    });
    // Dennis withdraws to 150 ONEU of debt, resulting in ICRs of 266%.
    const { collateral: D_coll } = await openTrove({
      ICR: toBN(dec(266, 16)),
    ...
    // Dennis redeems 40, so Bob has a surplus of (200 * 1 - 40) / 200 = 0.8 AUT
    ...
    // Bob re-opens the trove, price 200, total debt 250 ONEU, ICR = 240% (lowest one)
  ...
  it("liquidateTroves(): With all ICRs > 110%, Liquidates Troves until system leaves recovery mode", async () => {
    ...
    // Everyone withdraws some ONEU from their Trove, resulting in different ICRs
    ...
    await openTrove({
      ICR: toBN(dec(400, 16)),
      extraONEUAmount: liquidationAmount,
      extraParams: { from: alice }
    });

    // Alice deposits ONEU to Stability Pool
    await stabilityPool.provideToSP(liquidationAmount, ZERO_ADDRESS, { from: alice });

    // price drops
    // price drops to 1AUT:90ONEU, reducing TCR below 150%
  ...
  it("liquidateTroves(): Liquidates Troves until 1) system has left recovery mode AND 2) it reaches a Trove with ICR >= 110%", async () => {
    ...
    // price drops to 1AUT:85ONEU, reducing TCR below 150%
  ...
  it("liquidateTroves(): a liquidation sequence containing Pool offsets increases the TCR", async () => {
    ...
    await openTrove({
      ICR: toBN(dec(198, 16)),
      extraONEUAmount: dec(101, 18),
      extraParams: { from: defaulter_1 }
    });
    await openTrove({
      ICR: toBN(dec(184, 16)),
      extraONEUAmount: dec(217, 18),
      extraParams: { from: defaulter_2 }
    });
    await openTrove({
      ICR: toBN(dec(183, 16)),
      extraONEUAmount: dec(328, 18),
      extraParams: { from: defaulter_3 }
    });
    await openTrove({
      ICR: toBN(dec(186, 16)),
      extraONEUAmount: dec(431, 18),
    ...
    // Check Stability Pool has 500 ONEU
  ...
  it("liquidateTroves(): does nothing if all troves have ICR > 110% and Stability Pool is empty", async () => {
    ...
    // Confirm 0 ONEU in Stability Pool
    assert.equal((await stabilityPool.getTotalONEUDeposits()).toString(), "0");
  ...
  it("liquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves", async () => {
    ...
    // Confirm ONEU in Stability Pool
    assert.equal((await stabilityPool.getTotalONEUDeposits()).toString(), spDeposit.toString());
  ...
  it("liquidateTroves():  emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves, including a partial", async () => {
    ...
    const entireColl_A = (await troveManager.Troves(alice))[1].add(
      await troveManager.getPendingAUTReward(alice)
    );
    const entireDebt_A = (await troveManager.Troves(alice))[0].add(
      await troveManager.getPendingONEUDebtReward(alice)
    );
  ...
  it("liquidateTroves(): does not affect the liquidated user's token balances", async () => {
    ...
    const { oneuAmount: oneuAmountD } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraParams: { from: dennis }
    });
    const { oneuAmount: oneuAmountE } = await openTrove({
      ICR: toBN(dec(133, 16)),
      extraParams: { from: erin }
    });
    const { oneuAmount: oneuAmountF } = await openTrove({
      ICR: toBN(dec(111, 16)),
      extraParams: { from: freddy }
    });
    ...
    assert.equal((await oneuToken.balanceOf(dennis)).toString(), oneuAmountD);
    assert.equal((await oneuToken.balanceOf(erin)).toString(), oneuAmountE);
    assert.equal((await oneuToken.balanceOf(freddy)).toString(), oneuAmountF);
  ...
  it("liquidateTroves(): Liquidating troves at 100 < ICR < 110 with SP deposits correctly impacts their SP deposit and AUT gain", async () => {
    // Whale provides ONEU to the SP
    const { oneuAmount: W_oneuAmount } = await openTrove({
      ICR: toBN(dec(300, 16)),
      extraONEUAmount: dec(4000, 18),
      extraParams: { from: whale }
    });
    await stabilityPool.provideToSP(W_oneuAmount, ZERO_ADDRESS, { from: whale });

    const { oneuAmount: A_oneuAmount, totalDebt: A_totalDebt, collateral: A_coll } = await openTrove(
      {
        ICR: toBN(dec(191, 16)),
    ...
    const { oneuAmount: B_oneuAmount, totalDebt: B_totalDebt, collateral: B_coll } = await openTrove(
      {
        ICR: toBN(dec(200, 16)),
        extraONEUAmount: dec(240, 18),
        extraParams: { from: bob }
    ...
    await stabilityPool.provideToSP(A_oneuAmount, ZERO_ADDRESS, { from: alice });
    await stabilityPool.provideToSP(B_oneuAmount, ZERO_ADDRESS, { from: bob });

    const totalDeposit = W_oneuAmount.add(A_oneuAmount).add(B_oneuAmount);
    ...
    // Check ONEU in Pool
    assert.equal((await stabilityPool.getTotalONEUDeposits()).toString(), totalDeposit);
    ...
    Total ONEU in Pool: 680 ONEU

    Then, liquidation hits A,B,C: 

    Total liquidated debt = 100 + 300 + 100 = 500 ONEU
    Total liquidated AUT = 1 + 3 + 1 = 5 AUT

    Whale ONEU Loss: 500 * (400/680) = 294.12 ONEU
    Alice ONEU Loss:  500 *(40/680) = 29.41 ONEU
    Bob ONEU Loss: 500 * (240/680) = 176.47 ONEU

    Whale remaining deposit: (400 - 294.12) = 105.88 ONEU
    Alice remaining deposit: (40 - 29.41) = 10.59 ONEU
    Bob remaining deposit: (240 - 176.47) = 63.53 ONEU

    Whale AUT Gain: 5*0.995 * (400/680) = 2.93 AUT
    Alice AUT Gain: 5*0.995 *(40/680) = 0.293 AUT
    Bob AUT Gain: 5*0.995 * (240/680) = 1.76 AUT

    Total remaining deposits: 180 ONEU
    Total AUT gain: 5*0.995 AUT */
    ...
    const whale_Deposit_After = (await stabilityPool.getCompoundedONEUDeposit(whale)).toString();
    ...
    const whale_AUTGain = (await stabilityPool.getDepositorAUTGain(whale)).toString();
    const alice_AUTGain = (await stabilityPool.getDepositorAUTGain(alice)).toString();
    const bob_AUTGain = (await stabilityPool.getDepositorAUTGain(bob)).toString();
    ...
    assert.isAtMost(
      th.getDifference(
        whale_Deposit_After,
        W_oneuAmount.sub(liquidatedDebt.mul(W_oneuAmount).div(totalDeposit))
      ),
      100000
    );
    assert.isAtMost(
      th.getDifference(
        alice_Deposit_After,
        A_oneuAmount.sub(liquidatedDebt.mul(A_oneuAmount).div(totalDeposit))
      ),
      100000
    );
    assert.isAtMost(
      th.getDifference(
        bob_Deposit_After,
        B_oneuAmount.sub(liquidatedDebt.mul(B_oneuAmount).div(totalDeposit))
      ),
      100000
    );
    ...
    assert.isAtMost(
      th.getDifference(
        whale_AUTGain,
        th.applyLiquidationFee(liquidatedColl).mul(W_oneuAmount).div(totalDeposit)
      ),
      2000
    );
    assert.isAtMost(
      th.getDifference(
        alice_AUTGain,
        th.applyLiquidationFee(liquidatedColl).mul(A_oneuAmount).div(totalDeposit)
      ),
      2000
    );
    assert.isAtMost(
      th.getDifference(
        bob_AUTGain,
        th.applyLiquidationFee(liquidatedColl).mul(B_oneuAmount).div(totalDeposit)
      ),
      2000
    );
    ...
    assert.isAtMost(th.getDifference(total_ONEUinSP, totalDeposit.sub(liquidatedDebt)), 1000);
    assert.isAtMost(th.getDifference(total_AUTinSP, th.applyLiquidationFee(liquidatedColl)), 1000);
  });

  it("liquidateTroves(): Liquidating troves at ICR <=100% with SP deposits does not alter their deposit or AUT gain", async () => {
    ...
    // Check ONEU and AUT in Pool  before
    ...
    assert.equal(ONEUinSP_Before, dec(800, 18));
    assert.equal(AUTinSP_Before, "0");
    ...
    // Check ONEU and AUT in Pool after
    ...
    const whale_AUTGain_After = (await stabilityPool.getDepositorAUTGain(whale)).toString();
    ...
    assert.equal(whale_AUTGain_After, "0");
    assert.equal(alice_AUTGain_After, "0");
    assert.equal(bob_AUTGain_After, "0");
  });
  ...
  it("liquidateTroves() with a non fullfilled liquidation: non liquidated trove remains active", async () => {
    ...
    That leaves 50 ONEU in the Pool to absorb exactly half of Carol's debt (100) */
    await troveManager.liquidateTroves(10);
  ...
  it("liquidateTroves() with a non fullfilled liquidation: still can liquidate further troves after the non-liquidated, emptied pool", async () => {
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
     ...
     That leaves 97 ONEU in the Pool that won’t be enough to absorb Carol,
  ...
  it("liquidateTroves() with a non fullfilled liquidation: total liquidated coll and debt is correct", async () => {
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
    With 253 in the SP, Alice (102 debt) and Bob (101 debt) should be entirely liquidated. 
    That leaves 50 ONEU in the Pool that won’t be enough to absorb any other trove */
    const tx = await troveManager.liquidateTroves(10);

    // Expect system debt reduced by 203 ONEU and system coll 2.3 AUT
    const entireSystemCollAfter = await troveManager.getEntireSystemColl();
    const entireSystemDebtAfter = await troveManager.getEntireSystemDebt();
  ...
  it("liquidateTroves() with a non fullfilled liquidation: emits correct liquidation event values", async () => {
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
    With 253 in the SP, Alice (102 debt) and Bob (101 debt) should be entirely liquidated. 
    That leaves 50 ONEU in the Pool which won’t be enough for any other liquidation */
    const liquidationTx = await troveManager.liquidateTroves(10);
    ...
    assert.equal(oneuGasComp.toString(), dec(400, 18));
  ...
  it("batchLiquidateTroves(): Liquidates all troves with ICR < 110%, transitioning Normal -> Recovery Mode", async () => {
    ...
    await oneuToken.transfer(alice, A_totalDebt, { from: whale });
    // Alice closes trove
  ...
  it("batchLiquidateTroves() with a non fullfilled liquidation: total liquidated coll and debt is correct", async () => {
    ...
    // Expect system debt reduced by 203 ONEU and system coll by 2 AUT
    const entireSystemCollAfter = await troveManager.getEntireSystemColl();
    const entireSystemDebtAfter = await troveManager.getEntireSystemDebt();
  ...
  it("batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool ONEU > debt to liquidate: can liquidate troves out of order", async () => {
    const { totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraParams: { from: alice }
    });
    ...
    // Whale provides 1000 ONEU to the SP
  ...
  it("batchLiquidateTroves(): skips liquidation of troves with ICR > TCR, regardless of Stability Pool size", async () => {
    ...
    // Whale withdraws entire deposit, and re-deposits 132 ONEU
    ...
    // Expected Stability Pool to fully absorb B (92 ONEU + 10 virtual debt),
    // but not E as there are not enough funds in Stability Pool

    const stabilityBefore = await stabilityPool.getTotalONEUDeposits();
    ...
    const stabilityAfter = await stabilityPool.getTotalONEUDeposits();
  ...
  it("batchLiquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves, including a partial", async () => {
    ...
    // Whale opens trove and adds 220 ONEU to SP
    const spDeposit = F_totalDebt.add(G_totalDebt).add(A_totalDebt.div(toBN(2)));
    await openTrove({
      ICR: toBN(dec(285, 16)),
      extraONEUAmount: spDeposit,
