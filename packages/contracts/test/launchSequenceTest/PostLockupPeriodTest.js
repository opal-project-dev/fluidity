...
contract("After the initial lockup period has passed", async accounts => {
  ...
  describe("Beneficiary withdrawal from initial LC", async accounts => {
    ...
    it("A beneficiary on a vesting schedule can withdraw their total vested amount from their LC", async () => {
      ...
      const OPLBalanceOfLC_T1_Before = await lqtyToken.balanceOf(LC_T1.address);
      const OPLBalanceOfLC_T2_Before = await lqtyToken.balanceOf(LC_T2.address);
      const OPLBalanceOfLC_T3_Before = await lqtyToken.balanceOf(LC_T3.address);
      ...
      assert.isTrue((await lqtyToken.balanceOf(teamMember_1)).eq(OPLBalanceOfLC_T1_Before));
      assert.isTrue((await lqtyToken.balanceOf(teamMember_2)).eq(OPLBalanceOfLC_T2_Before));
      assert.isTrue((await lqtyToken.balanceOf(teamMember_3)).eq(OPLBalanceOfLC_T3_Before));
    ...
    it("Beneficiaries can withraw full OPL balance of LC if it has increased since lockup period ended", async () => {
      ...
      const OPLBalanceOfLC_I1_Before = await lqtyToken.balanceOf(LC_I1.address);
      const OPLBalanceOfLC_I2_Before = await lqtyToken.balanceOf(LC_I2.address);
      const OPLBalanceOfLC_I3_Before = await lqtyToken.balanceOf(LC_I3.address);
      ...
      assert.isTrue((await lqtyToken.balanceOf(investor_1)).eq(OPLBalanceOfLC_I1_Before));
      assert.isTrue((await lqtyToken.balanceOf(investor_2)).eq(OPLBalanceOfLC_I2_Before));
      assert.isTrue((await lqtyToken.balanceOf(investor_3)).eq(OPLBalanceOfLC_I3_Before));
  ...
  describe("Transferring OPL", async accounts => {
    it("OPL multisig can transfer OPL to LCs they deployed", async () => {
      const initialOPLBalanceOfLC_T1 = await lqtyToken.balanceOf(LC_T1.address);
      const initialOPLBalanceOfLC_T2 = await lqtyToken.balanceOf(LC_T2.address);
      const initialOPLBalanceOfLC_T3 = await lqtyToken.balanceOf(LC_T3.address);
      ...
      const OPLBalanceOfLC_T1_1 = await lqtyToken.balanceOf(LC_T1.address);
      const OPLBalanceOfLC_T2_1 = await lqtyToken.balanceOf(LC_T2.address);
      const OPLBalanceOfLC_T3_1 = await lqtyToken.balanceOf(LC_T3.address);
      ...
      // Get new LC OPL balances
      const OPLBalanceOfLC_T1_2 = await lqtyToken.balanceOf(LC_T1.address);
      const OPLBalanceOfLC_T2_2 = await lqtyToken.balanceOf(LC_T2.address);
      const OPLBalanceOfLC_T3_2 = await lqtyToken.balanceOf(LC_T3.address);
    ...
    it("Anyone can transfer to an EOA", async () => {
      ...
      const OPLtransferTx_1 = await lqtyToken.transfer(A, dec(1, 18), { from: D });
      const OPLtransferTx_2 = await lqtyToken.transfer(liquityAG, dec(1, 18), { from: E });
      const OPLtransferTx_3 = await lqtyToken.transfer(F, dec(1, 18), { from: liquityAG });
      const OPLtransferTx_4 = await lqtyToken.transfer(G, dec(1, 18), { from: multisig });
    ...
    it("Anyone can approve any EOA to spend their OPL", async () => {
      ...
      const OPLapproveTx_1 = await lqtyToken.approve(A, dec(1, 18), { from: multisig });
      const OPLapproveTx_2 = await lqtyToken.approve(B, dec(1, 18), { from: G });
      const OPLapproveTx_3 = await lqtyToken.approve(liquityAG, dec(1, 18), { from: F });
    ...
    it("Anyone can increaseAllowance for any EOA or Liquity contract", async () => {
      ...
      const OPLIncreaseAllowanceTx_1 = await lqtyToken.increaseAllowance(A, dec(1, 18), {
        from: multisig
      });
      const OPLIncreaseAllowanceTx_2 = await lqtyToken.increaseAllowance(B, dec(1, 18), { from: G });
      const OPLIncreaseAllowanceTx_3 = await lqtyToken.increaseAllowance(multisig, dec(1, 18), {
        from: F
      });
      ...
      for (const contract of Object.keys(coreContracts)) {
        const OPLIncreaseAllowanceTx = await lqtyToken.increaseAllowance(
    ...
    it("Anyone can decreaseAllowance for any EOA or Liquity contract", async () => {
      ...
      const OPLapproveTx_1 = await lqtyToken.approve(A, dec(1, 18), { from: multisig });
      const OPLapproveTx_2 = await lqtyToken.approve(B, dec(1, 18), { from: G });
      const OPLapproveTx_3 = await lqtyToken.approve(multisig, dec(1, 18), { from: F });
      await assert.isTrue(OPLapproveTx_1.receipt.status);
      await assert.isTrue(OPLapproveTx_2.receipt.status);
      await assert.isTrue(OPLapproveTx_3.receipt.status);
      ...
      for (const contract of Object.keys(coreContracts)) {
        const OPLtransferTx = await lqtyToken.increaseAllowance(
          coreContracts[contract].address,
          dec(1, 18),
          { from: F }
        );
      ...
      const OPLDecreaseAllowanceTx_1 = await lqtyToken.decreaseAllowance(A, dec(1, 18), {
        from: multisig
      });
      const OPLDecreaseAllowanceTx_2 = await lqtyToken.decreaseAllowance(B, dec(1, 18), { from: G });
      const OPLDecreaseAllowanceTx_3 = await lqtyToken.decreaseAllowance(multisig, dec(1, 18), {
        from: F
      });
      ...
      for (const contract of Object.keys(coreContracts)) {
        const OPLDecreaseAllowanceTx = await lqtyToken.decreaseAllowance(
          coreContracts[contract].address,
          dec(1, 18),
          { from: F }
        );
      ...
      for (const contract of Object.keys(OPLContracts)) {
        const OPLIncreaseAllowanceTx = await lqtyToken.decreaseAllowance(
          OPLContracts[contract].address,
          dec(1, 18),
          { from: F }
        );
    ...
    it("Anyone can be the sender in a transferFrom() call", async () => {
      ...
      const OPLtransferFromTx_1 = await lqtyToken.transferFrom(multisig, F, dec(1, 18), { from: F });
      const OPLtransferFromTx_2 = await lqtyToken.transferFrom(B, multisig, dec(1, 18), { from: G });
      const OPLtransferFromTx_3 = await lqtyToken.transferFrom(C, A, dec(1, 18), { from: multisig });
    ...
    it("Anyone can stake their OPL in the staking contract", async () => {
      ...
      const OPLStakingTx_1 = await lqtyStaking.stake(dec(1, 18), { from: F });
      const OPLStakingTx_2 = await lqtyStaking.stake(dec(1, 18), { from: multisig });
