...
contract("Gas compensation tests", async accounts => {
  ...
  it("_getCollGasCompensation(): returns the 0.5% of collaterall if it is < $10 in value", async () => {
    ...
    /* 
    AUT:USD price = 1000000000 (1 billion)
    coll = 0.000000005 AUT (5e9 wei): $5 in value 
    -> Expect 0.5% of collaterall as gas compensation */
  ...
  it("getCollGasCompensation(): returns 0.5% of collaterall when 0.5% of collateral = $10 in value", async () => {
    ...
    AUT:USD price = 200 $/E
    coll = 100 AUT  
    0.5% of coll = 0.5 AUT. USD value: $100
    -> Expect $100 gas compensation, i.e. 0.5 AUT */
    const gasCompensation_1 = (
    ...
    AUT:USD price = 200 $/E
    coll = 10.001 AUT  
    0.5% of coll = 0.050005 AUT. USD value: $10.001
    -> Expect $100 gas compensation, i.e.  0.050005  AUT */
    const gasCompensation_2 = (
    ...
    AUT:USD price = 200 $/E
    coll = 37.5 AUT  
    0.5% of coll = 0.1875 AUT. USD value: $37.5
    -> Expect $37.5 gas compensation i.e.  0.1875  AUT */
    const gasCompensation_3 = (
    ...
    0.5% of coll = 473.7911529 AUT. USD value: $21473894.84
    -> Expect $21473894.8385808 gas compensation, i.e.  473.7911529115490  AUT */
    ...
    0.5% of coll = 1500000 AUT. USD value: $150000000000
    -> Expect $150000000000 gas compensation, i.e. 1500000 AUT */
  ...
  // returns $10 worth of AUT when 0.5% of coll == $10
  ...
  it("Gas compensation from pool-offset liquidations. All collateral paid as compensation", async () => {
    ...
    // Check liquidator's balance increases by 0.5% of A's coll (1 AUT)
    ...
    assert.equal(AUTinSP_A.toString(), aliceColl.sub(_0pt5percent_aliceColl)); // 1 AUT - 0.5%
    ...
    // Check liquidator's balance increases by B's 0.5% of coll, 2 AUT
    ...
    /* 
    AUT:USD price = 3.141592653589793238
    Carol coll = 3 AUT. Value = (3 * 3.141592653589793238) = $6
    -> Expect 0.5% of collaterall to be sent to liquidator, as gas compensation */
    ...
    // Check liquidator's balance increases by C's 0.5% of coll, 3 AUT
  ...
  it("gas compensation from pool-offset liquidations: 0.5% collateral < $10 in value. Compensates $10 worth of collateral, liquidates the remainder", async () => {
    ...
    /* 
    AUT:USD price = 199.999
    Alice coll = 1 AUT. Value: $199.999
    0.5% of coll  = 0.05 AUT. Value: (0.05 * 199.999) = $9.99995
    Minimum comp = $10 = 0.05000025000125001 AUT.
    -> Expect 0.05000025000125001 AUT sent to liquidator, 
    and (1 - 0.05000025000125001) = 0.94999974999875 AUT remainder liquidated */
    ...
    const SPAUTIncrease_A = AUTinSP_A.sub(AUTinSP_0);

    assert.isAtMost(th.getDifference(SPAUTIncrease_A, collRemainder_A), 1000);
    ...
    /* 
    AUT:USD price = 15
    Bob coll = 15 AUT. Value: $165
    0.5% of coll  = 0.75 AUT. Value: (0.75 * 11) = $8.25
    Minimum comp = $10 =  0.66666...AUT.
    -> Expect 0.666666666666666666 AUT sent to liquidator, 
    and (15 - 0.666666666666666666) AUT remainder liquidated */
    ...
    const SPAUTIncrease_B = AUTinSP_B.sub(AUTinSP_A);

    assert.isAtMost(th.getDifference(SPAUTIncrease_B, collRemainder_B), 1000);
  });
  ...
  it("gas compensation from pool-offset liquidations: 0.5% collateral > $10 in value. Compensates 0.5% of  collateral, liquidates the remainder", async () => {
    ...
    /* 
    AUT:USD price = 200
    Alice coll = 10.001 AUT. Value: $2000.2
    0.5% of coll  = 0.050005 AUT. Value: (0.050005 * 200) = $10.01
    Minimum comp = $10 = 0.05 AUT.
    -> Expect  0.050005 AUT sent to liquidator, 
    and (10.001 - 0.050005) AUT remainder liquidated */
    ...
    /* 
   AUT:USD price = 200
   Bob coll = 37.5 AUT. Value: $7500
   0.5% of coll  = 0.1875 AUT. Value: (0.1875 * 200) = $37.5
   Minimum comp = $10 = 0.05 AUT.
   -> Expect 0.1875 AUT sent to liquidator, 
   and (37.5 - 0.1875 AUT) AUT remainder liquidated */
  ...
  it("liquidateTroves(): full offset.  Compensates the correct amount, and liquidates the remainder", async () => {
    ...
    */
    ...
    // Check AUT in stability pool now equals the expected liquidated collateral
    const AUTinSP = (await stabilityPool.getAUT()).toString();
    assert.equal(expectedLiquidatedColl, AUTinSP);
  });
  ...
  it("liquidateTroves(): full redistribution. Compensates the correct amount, and liquidates the remainder", async () => {
    ...
    */
    ...
    // Check AUT in defaultPool now equals the expected liquidated collateral
    const AUTinDefaultPool = (await defaultPool.getAUT()).toString();
    assert.isAtMost(th.getDifference(expectedLiquidatedColl, AUTinDefaultPool), 1000);
  });
      ...
      for (account of _10_accounts) {
        ...
        if (ICRList.length > 1) {
          ...
          try {
            ...
            console.log(`AUT price at which trove ordering breaks: ${price}`);
            logICRs(ICRList);
          }
        }
