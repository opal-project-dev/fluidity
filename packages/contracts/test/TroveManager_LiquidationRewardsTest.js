...
contract("TroveManager - Redistribution reward calculations", async accounts => {
  ...
  it("redistribution: A, B Open. B Liquidated. C, D Open. D Liquidated. Distributes correct rewards", async () => {
    ...
    const alice_Coll = (await troveManager.Troves(alice))[1]
      .add(await troveManager.getPendingAUTReward(alice))
      .toString();
    const carol_Coll = (await troveManager.Troves(carol))[1]
      .add(await troveManager.getPendingAUTReward(carol))
      .toString();

    /* Expected collateral:
    A: Alice receives 0.995 AUT from L1, and ~3/5*0.995 AUT from L2.
    expect aliceColl = 2 + 0.995 + 2.995/4.995 * 0.995 = 3.5916 AUT

    C: Carol receives ~2/5 AUT from L2
    expect carolColl = 2 + 2/4.995 * 0.995 = 2.398 AUT

    Total coll = 4 + 2 * 0.995 AUT
    */
    ...
    const entireSystemColl = (await activePool.getAUT()).add(await defaultPool.getAUT()).toString();
    assert.equal(
      entireSystemColl,
      A_coll.add(C_coll).add(th.applyLiquidationFee(B_coll.add(D_coll)))
    );

    // check ONEU gas compensation
    assert.equal((await oneuToken.balanceOf(owner)).toString(), dec(400, 18));
  });
  ...
  it("redistribution: A, B, C Open. C Liquidated. D, E, F Open. F Liquidated. Distributes correct rewards", async () => {
    ...
    const bob_Coll = (await troveManager.Troves(bob))[1]
      .add(await troveManager.getPendingAUTReward(bob))
      .toString();
    const dennis_Coll = (await troveManager.Troves(dennis))[1]
      .add(await troveManager.getPendingAUTReward(dennis))
      .toString();
    const erin_Coll = (await troveManager.Troves(erin))[1]
      .add(await troveManager.getPendingAUTReward(erin))
      .toString();

    /* Expected collateral:
    A and B receives 1/2 AUT * 0.995 from L1.
    total Coll: 3

    A, B, receive (2.4975)/8.995 * 0.995 AUT from L2.
    
    D, E receive 2/8.995 * 0.995 AUT from L2.

    expect A, B coll  = 2 +  0.4975 + 0.2763  =  AUT
    expect D, E coll  = 2 + 0.2212  =  AUT
  ...
  it("redistribution: Sequence of alternate opening/liquidation: final surviving trove has AUT from all previously liquidated troves", async () => {
    // A, B  open troves
    const { collateral: A_coll } = await openTrove({
      ICR: toBN(dec(400, 16)),
      extraParams: { from: alice }
    ...
    const freddy_AUTReward = (await troveManager.getPendingAUTReward(freddy)).toString();

    /* Expected collateral:
     A-E should have been liquidated
     trove F should have acquired all AUT in the system: 1 AUT initial coll, and 0.995^5+0.995^4+0.995^3+0.995^2+0.995 from rewards = 5.925 AUT
    */
    ...
    const gainedAUT = th.applyLiquidationFee(
      E_coll.add(
    ...
    assert.isAtMost(th.getDifference(freddy_AUTReward, gainedAUT), 1000);

    const entireSystemColl = (await activePool.getAUT()).add(await defaultPool.getAUT()).toString();
    assert.isAtMost(th.getDifference(entireSystemColl, F_coll.add(gainedAUT)), 1000);

    // check ONEU gas compensation
    assert.equal((await oneuToken.balanceOf(owner)).toString(), dec(1000, 18));
  });
  ...
  it("redistribution: A,B,C,D open. Liq(A). B adds coll. Liq(C). B and D have correct coll and debt", async () => {
    ...
    // Check accumulated AUT gain for each trove
    const B_AUTGain_1 = await troveManager.getPendingAUTReward(B);
    const C_AUTGain_1 = await troveManager.getPendingAUTReward(C);
    const D_AUTGain_1 = await troveManager.getPendingAUTReward(D);
    const E_AUTGain_1 = await troveManager.getPendingAUTReward(E);

    // Check gains are what we'd expect from a distribution proportional to each trove's entire coll
    const B_expectedPendingAUT_1 = A_collRedistribution.mul(B_entireColl_0).div(denominatorColl_1);
    const C_expectedPendingAUT_1 = A_collRedistribution.mul(C_entireColl_0).div(denominatorColl_1);
    const D_expectedPendingAUT_1 = A_collRedistribution.mul(D_entireColl_0).div(denominatorColl_1);
    const E_expectedPendingAUT_1 = A_collRedistribution.mul(E_entireColl_0).div(denominatorColl_1);

    assert.isAtMost(getDifference(B_expectedPendingAUT_1, B_AUTGain_1), 1e8);
    assert.isAtMost(getDifference(C_expectedPendingAUT_1, C_AUTGain_1), 1e8);
    assert.isAtMost(getDifference(D_expectedPendingAUT_1, D_AUTGain_1), 1e8);
    assert.isAtMost(getDifference(E_expectedPendingAUT_1, E_AUTGain_1), 1e8);
    ...
    const B_AUTGain_2 = await troveManager.getPendingAUTReward(B);
    const D_AUTGain_2 = await troveManager.getPendingAUTReward(D);
    const E_AUTGain_2 = await troveManager.getPendingAUTReward(E);

    // Since B topped up, he has no previous pending AUT gain
    const B_expectedPendingAUT_2 = C_collRedistribution.mul(B_entireColl_1).div(denominatorColl_2);

    // D & E's accumulated pending AUT gain includes their previous gain
    const D_expectedPendingAUT_2 = C_collRedistribution.mul(D_entireColl_1)
      .div(denominatorColl_2)
      .add(D_expectedPendingAUT_1);

    const E_expectedPendingAUT_2 = C_collRedistribution.mul(E_entireColl_1)
      .div(denominatorColl_2)
      .add(E_expectedPendingAUT_1);

    assert.isAtMost(getDifference(B_expectedPendingAUT_2, B_AUTGain_2), 1e8);
    assert.isAtMost(getDifference(D_expectedPendingAUT_2, D_AUTGain_2), 1e8);
    assert.isAtMost(getDifference(E_expectedPendingAUT_2, E_AUTGain_2), 1e8);
    ...
    const B_AUTGain_3 = await troveManager.getPendingAUTReward(B);
    const D_AUTGain_3 = await troveManager.getPendingAUTReward(D);

    // Since B topped up, he has no previous pending AUT gain
    const B_expectedPendingAUT_3 = E_collRedistribution.mul(B_entireColl_2).div(denominatorColl_3);

    // D'S accumulated pending AUT gain includes their previous gain
    const D_expectedPendingAUT_3 = E_collRedistribution.mul(D_entireColl_2)
      .div(denominatorColl_3)
      .add(D_expectedPendingAUT_2);

    assert.isAtMost(getDifference(B_expectedPendingAUT_3, B_AUTGain_3), 1e8);
    assert.isAtMost(getDifference(D_expectedPendingAUT_3, D_AUTGain_3), 1e8);
  });
  ...
  it("redistribution: A,B,C Open. Liq(C). B adds coll. Liq(A). B acquires all coll and debt", async () => {
    ...
    // Expect Bob now holds all Ether and ONEUDebt in the system: 2 + 0.4975+0.4975*0.995+0.995 Ether and 110*3 ONEU (10 each for gas compensation)
    const bob_Coll = (await troveManager.Troves(bob))[1]
      .add(await troveManager.getPendingAUTReward(bob))
      .toString();

    const bob_ONEUDebt = (await troveManager.Troves(bob))[0]
      .add(await troveManager.getPendingONEUDebtReward(bob))
      .toString();
    ...
    assert.isAtMost(
      th.getDifference(bob_ONEUDebt, A_totalDebt.mul(toBN(2)).add(B_totalDebt).add(C_totalDebt)),
      1000
    );
  });
  ...
  it("redistribution: A,B,C Open. Liq(C). B tops up coll. D Opens. Liq(D). Distributes correct rewards.", async () => {
    ...
    /* Bob rewards:
     L1: 1/2*0.995 AUT, 55 ONEU
     L2: (2.4975/3.995)*0.995 = 0.622 AUT , 110*(2.4975/3.995)= 68.77 ONEUDebt

    coll: 3.1195 AUT
    debt: 233.77 ONEUDebt

     Alice rewards:
    L1 1/2*0.995 AUT, 55 ONEU
    L2 (1.4975/3.995)*0.995 = 0.3730 AUT, 110*(1.4975/3.995) = 41.23 ONEUDebt

    coll: 1.8705 AUT
    debt: 146.23 ONEUDebt

    totalColl: 4.99 AUT
    totalDebt 380 ONEU (includes 50 each for gas compensation)
    */
    const bob_Coll = (await troveManager.Troves(bob))[1]
      .add(await troveManager.getPendingAUTReward(bob))
      .toString();
    ...
    const alice_ONEUDebt = (await troveManager.Troves(alice))[0]
      .add(await troveManager.getPendingONEUDebtReward(alice))
      .toString();
    ...
    assert.isAtMost(th.getDifference(bob_Coll, expected_B_coll), 1000);
    assert.isAtMost(th.getDifference(bob_ONEUDebt, expected_B_debt), 10000);
    ...
    assert.isAtMost(th.getDifference(alice_ONEUDebt, expected_A_debt), 10000);
  ...
  it("redistribution: Trove with the majority stake tops up. A,B,C, D open. Liq(D). C tops up. E Enters, Liq(E). Distributes correct rewards", async () => {
    ...
    // Expected rewards:  alice: 1 AUT, bob: 1 AUT, carol: 998 AUT
    const alice_AUTReward_1 = await troveManager.getPendingAUTReward(alice);
    const bob_AUTReward_1 = await troveManager.getPendingAUTReward(bob);
    const carol_AUTReward_1 = await troveManager.getPendingAUTReward(carol);

    //Expect 1000 + 1000*0.995 AUT in system now
    const entireSystemColl_1 = (await activePool.getAUT())
      .add(await defaultPool.getAUT())
      .toString();
    assert.equal(
      entireSystemColl_1,
    ...
    /* Expected AUT rewards: 
     Carol = 1992.01/1996 * 1996*0.995 = 1982.05 AUT
     Alice = 1.995/1996 * 1996*0.995 = 1.985025 AUT
     Bob = 1.995/1996 * 1996*0.995 = 1.985025 AUT

    therefore, expected total collateral:

    Carol = 1991.01 + 1991.01 = 3974.06
    Alice = 1.995 + 1.985025 = 3.980025 AUT
    Bob = 1.995 + 1.985025 = 3.980025 AUT
    ...
    */
    ...
    //Expect 3982.02 AUT in system now
    const entireSystemColl_3 = (await activePool.getAUT())
      .add(await defaultPool.getAUT())
      .toString();
    th.assertIsApproximatelyEqual(
      entireSystemColl_3,
    ...
    // check ONEU gas compensation
    th.assertIsApproximatelyEqual((await oneuToken.balanceOf(owner)).toString(), dec(400, 18));
  });
  ...
  it("redistribution: Trove with the majority stake tops up. A,B,C, D open. Liq(D). A, B, C top up. E Enters, Liq(E). Distributes correct rewards", async () => {
    ...
    // Expected rewards:  alice: 1 AUT, bob: 1 AUT, carol: 998 AUT (*0.995)
    const alice_AUTReward_1 = await troveManager.getPendingAUTReward(alice);
    const bob_AUTReward_1 = await troveManager.getPendingAUTReward(bob);
    const carol_AUTReward_1 = await troveManager.getPendingAUTReward(carol);

    //Expect 1995 AUT in system now
    const entireSystemColl_1 = (await activePool.getAUT())
      .add(await defaultPool.getAUT())
      .toString();
    assert.equal(
    ...
    /* Alice, Bob, Carol each adds 1 AUT to their troves, 
    ...
    //Expect 1998 AUT in system now
    const entireSystemColl_2 = (await activePool.getAUT())
      .add(await defaultPool.getAUT())
      .toString();
    th.assertIsApproximatelyEqual(
      entireSystemColl_2,
    ...
    /* Expected AUT rewards: 
     Carol = 1992.01/1998 * 1998*0.995 = 1982.04995 AUT
     Alice = 2.995/1998 * 1998*0.995 = 2.980025 AUT
     Bob = 2.995/1998 * 1998*0.995 = 2.980025 AUT

    therefore, expected total collateral:

    Carol = 1992.01 + 1982.04995 = 3974.05995
    Alice = 2.995 + 2.980025 = 5.975025 AUT
    Bob = 2.995 + 2.980025 = 5.975025 AUT
    ...
    */
    ...
    //Expect 3986.01 AUT in system now
    const entireSystemColl_3 = (await activePool.getAUT()).add(await defaultPool.getAUT());
    th.assertIsApproximatelyEqual(
      entireSystemColl_3,
      totalCollAfterL1.add(th.applyLiquidationFee(E_coll))
    );
  ...
  it("redistribution: A,B,C Open. Liq(C). B withdraws coll. Liq(A). B acquires all coll and debt", async () => {
    ...
    //Bob withdraws 0.5 AUT from his trove
    ...
    // Expect Bob now holds all Ether and ONEUDebt in the system: 2.5 Ether and 300 ONEU
  ...
  it("redistribution: A,B,C Open. Liq(C). B withdraws coll. D Opens. Liq(D). Distributes correct rewards.", async () => {
    ...
    /* Bob rewards:
     L1: 0.4975 AUT, 55 ONEU
     L2: (0.9975/2.495)*0.995 = 0.3978 AUT , 110*(0.9975/2.495)= 43.98 ONEUDebt

    coll: (1 + 0.4975 - 0.5 + 0.3968) = 1.3953 AUT
    debt: (110 + 55 + 43.98 = 208.98 ONEUDebt 

     Alice rewards:
    L1 0.4975, 55 ONEU
    L2 (1.4975/2.495)*0.995 = 0.5972 AUT, 110*(1.4975/2.495) = 66.022 ONEUDebt

    coll: (1 + 0.4975 + 0.5972) = 2.0947 AUT
    debt: (50 + 55 + 66.022) = 171.022 ONEU Debt

    totalColl: 3.49 AUT
    totalDebt 380 ONEU (Includes 50 in each trove for gas compensation)
    */
    const bob_Coll = (await troveManager.Troves(bob))[1]
      .add(await troveManager.getPendingAUTReward(bob))
      .toString();
    ...
    const entireSystemColl = (await activePool.getAUT()).add(await defaultPool.getAUT());
    th.assertIsApproximatelyEqual(
      entireSystemColl,
      A_coll.add(B_coll)
        .add(th.applyLiquidationFee(C_coll))
    ...
    const entireSystemDebt = (await activePool.getONEUDebt()).add(await defaultPool.getONEUDebt());
    th.assertIsApproximatelyEqual(
      entireSystemDebt,
  ...
  it("redistribution: Trove with the majority stake withdraws. A,B,C,D open. Liq(D). C withdraws some coll. E Enters, Liq(E). Distributes correct rewards", async () => {
    ...
    //Expect 1995 AUT in system now
    const entireSystemColl_1 = (await activePool.getAUT()).add(await defaultPool.getAUT());
    th.assertIsApproximatelyEqual(
      entireSystemColl_1,
      A_coll.add(B_coll).add(C_coll).add(th.applyLiquidationFee(D_coll))
    );
    ...
    /* Expected AUT rewards: 
     Carol = 1990.01/1994 * 1994*0.995 = 1980.05995 AUT
     Alice = 1.995/1994 * 1994*0.995 = 1.985025 AUT
     Bob = 1.995/1994 * 1994*0.995 = 1.985025 AUT
    ...
    */
    ...
    //Expect 3978.03 AUT in system now
    const entireSystemColl_3 = (await activePool.getAUT()).add(await defaultPool.getAUT());
    th.assertIsApproximatelyEqual(
      entireSystemColl_3,
      totalCollAfterL1.add(th.applyLiquidationFee(E_coll))
  ...
  it("redistribution: Trove with the majority stake withdraws. A,B,C,D open. Liq(D). A, B, C withdraw. E Enters, Liq(E). Distributes correct rewards", async () => {
    ...
    /* Alice, Bob, Carol each withdraw 0.5 AUT to their troves, 
    ...
    //Expect 1993.5 AUT in system now
    const entireSystemColl_2 = (await activePool.getAUT()).add(await defaultPool.getAUT());
    th.assertIsApproximatelyEqual(
      entireSystemColl_2,
    ...
    // E opens with another 1993.5 AUT
    const { collateral: E_coll } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraParams: { from: erin, value: entireSystemColl_2 }
    });
    ...
    /* Expected AUT rewards: 
     Carol = 1990.51/1993.5 * 1993.5*0.995 = 1980.55745 AUT
     Alice = 1.495/1993.5 * 1993.5*0.995 = 1.487525 AUT
     Bob = 1.495/1993.5 * 1993.5*0.995 = 1.487525 AUT

    therefore, expected total collateral:

    Carol = 1990.51 + 1980.55745 = 3971.06745
    Alice = 1.495 + 1.487525 = 2.982525 AUT
    Bob = 1.495 + 1.487525 = 2.982525 AUT
    ...
    */
    ...
    //Expect 3977.0325 AUT in system now
    const entireSystemColl_3 = (await activePool.getAUT()).add(await defaultPool.getAUT());
    th.assertIsApproximatelyEqual(
      entireSystemColl_3,
      totalCollAfterL1.add(th.applyLiquidationFee(E_coll))
  ...
  it("redistribution, all operations: A,B,C open. Liq(A). D opens. B adds, C withdraws. Liq(B). E & F open. D adds. Liq(F). Distributes correct rewards", async () => {
    ...
    const C_pendingRewardsAfterL1 = th
      ...
      .div(B_coll.add(C_coll));
    assert.isAtMost(
      th.getDifference(await troveManager.getPendingAUTReward(bob), B_pendingRewardsAfterL1),
      1000000
    );
    assert.isAtMost(
      th.getDifference(await troveManager.getPendingAUTReward(carol), C_pendingRewardsAfterL1),
      1000000
    );
    ...
    //Carol  withdraws 1 AUT from her trove
    const C_withdrawnColl = toBN(dec(1, "ether"));
    await borrowerOperations.withdrawColl(C_withdrawnColl, carol, carol, { from: carol });
    ...
    assert.isAtMost(
      th.getDifference(await troveManager.getPendingAUTReward(carol), C_pendingRewardsAfterL2),
      1000000
    );
    assert.isAtMost(
      th.getDifference(await troveManager.getPendingAUTReward(dennis), D_pendingRewardsAfterL2),
      1000000
    );
    ...
    const carol_pendingAUTReward = (await troveManager.getPendingAUTReward(carol)).toString();

    const dennis_rawColl = (await troveManager.Troves(dennis))[1].toString();
    const dennis_pendingAUTReward = (await troveManager.getPendingAUTReward(dennis)).toString();

    const erin_rawColl = (await troveManager.Troves(erin))[1].toString();
    const erin_pendingAUTReward = (await troveManager.getPendingAUTReward(erin)).toString();
    ...
    // Check pending AUT rewards of C, D, E
    assert.isAtMost(
      th.getDifference(carol_pendingAUTReward, C_collAfterL3.sub(C_collAfterL1)),
      1000000
    );
    assert.isAtMost(
      th.getDifference(dennis_pendingAUTReward, D_collAfterL3.sub(D_collAfterL2)),
      1000000
    );
    assert.isAtMost(th.getDifference(erin_pendingAUTReward, E_collAfterL3.sub(E_coll)), 1000000);

    // Check systemic collateral
    const activeColl = (await activePool.getAUT()).toString();
    const defaultColl = (await defaultPool.getAUT()).toString();
    ...
    // check ONEU gas compensation
    assert.equal((await oneuToken.balanceOf(owner)).toString(), dec(600, 18));
  });
  ...
  it("redistribution, all operations: A,B,C open. Liq(A). D opens. B adds, C withdraws. Liq(B). E & F open. D adds. Liq(F). Varying coll. Distributes correct rewards", async () => {
    ...
    */
    ...
    // D opens trove: 0.035 AUT
    const { collateral: D_coll, totalDebt: D_totalDebt } = await openTrove({
      extraONEUAmount: dec(100, 18),
      extraParams: { from: dennis, value: toBN(dec(35, 15)) }
    });

    // Bob adds 11.33909 AUT to his trove
    const B_addedColl = toBN("11339090000000000000");
    await borrowerOperations.addColl(bob, bob, { from: bob, value: B_addedColl });

    // Carol withdraws 15 AUT from her trove
