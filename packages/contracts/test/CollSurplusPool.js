...
contract("CollSurplusPool", async accounts => {
  ...
  it("CollSurplusPool::getAUT(): Returns the AUT balance of the CollSurplusPool after redemption", async () => {
    const AUT_1 = await collSurplusPool.getAUT();
    assert.equal(AUT_1, "0");
    ...
    th.assertIsApproximatelyEqual(AUT_2, B_coll.sub(B_netDebt.mul(mv._1e18BN).div(price)));
  });
  ...
  it("CollSurplusPool: claimColl(): Reverts if owner cannot receive AUT surplus", async () => {
    ...
    await th.assertRevert(
      ...
      "CollSurplusPool: sending AUT failed"
    );
  });

  it("CollSurplusPool: reverts trying to send AUT to it", async () => {
    await th.assertRevert(
