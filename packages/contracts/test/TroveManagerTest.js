...
contract("TroveManager", async accounts => {
  ...
  it("liquidate(): closes a Trove that has ICR < MCR", async () => {
    ...
    // price drops to 1AUT:100LUSD, reducing Alice's ICR below MCR
  ...
  it("liquidate(): decreases ActivePool AUT and LUSDDebt by correct amounts", async () => {
    // --- SETUP ---
    ...
    // check ActivePool AUT and LUSD debt before
    ...
    // price drops to 1AUT:100LUSD, reducing Bob's ICR below MCR
    await priceFeed.setPrice("100000000000000000000");
  ...
  it("liquidate(): increases DefaultPool AUT and LUSD debt by correct amounts", async () => {
    // --- SETUP ---
    const { collateral: A_collateral, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(4, 18)),
      extraParams: { from: alice }
    ...
    // check DefaultPool AUT and LUSD debt before
  ...
  it("liquidate(): updates the L_AUT and L_LUSDDebt reward-per-unit-staked totals", async () => {
    // --- SETUP ---
    const { collateral: A_collateral, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(8, 18)),
      extraParams: { from: alice }
    ...
    // price drops to 1AUT:100LUSD, reducing Carols's ICR below MCR
    await priceFeed.setPrice("100000000000000000000");
    ...
    // price drops to 1AUT:50LUSD, reducing Bob's ICR below MCR
  ...
  it("liquidate(): Liquidates undercollateralized trove if there are two troves in the system", async () => {
    ...
    // Alice creates a single trove with 0.7 AUT and a debt of 70 LUSD, and provides 10 LUSD to SP
    const { collateral: A_collateral, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraParams: { from: alice }
    });
  ...
  it("liquidate(): liquidates a SP depositor's trove with ICR < 110%, and the liquidation correctly impacts their SP deposit and AUT gain", async () => {
    ...
    /* Alice's LUSD Loss = (300 / 400) * 200 = 150 LUSD
     ...
     Check Bob' SP deposit has been reduced to 50 LUSD, and his AUT gain has increased to 1.5 AUT. */
  ...
  it("liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    ...
    // Defaulter opens with 60 LUSD, 0.6 AUT
    ...
    /* Liquidate defaulter. 30 LUSD and 0.3 AUT is distributed between A, B and C.

    A receives (30 * 2/4) = 15 LUSD, and (0.3*2/4) = 0.15 AUT
    B receives (30 * 1/4) = 7.5 LUSD, and (0.3*1/4) = 0.075 AUT
    C receives (30 * 1/4) = 7.5 LUSD, and (0.3*1/4) = 0.075 AUT
    */
    await troveManager.liquidate(defaulter_1);
  ...
  it("liquidate(): when SP > 0, triggers LQTY reward event - increases the sum G", async () => {
    ...
    // Price drops to 1AUT:100LUSD, reducing defaulters to below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice();
    assert.isFalse(await th.checkRecoveryMode(contracts));
  ...
  it("liquidateTroves(): closes every Trove with ICR < MCR, when n > number of undercollateralized troves", async () => {
    ...
    // Price drops to 1AUT:100LUSD, reducing Bob and Carol's ICR below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice();
  ...
  it("liquidateTroves(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    ...
    // Liquidate defaulter. 30 LUSD and 0.3 AUT is distributed uniformly between A, B and C. Each receive 10 LUSD, 0.1 AUT
    await troveManager.liquidate(defaulter_1);
  ...
  it("liquidateTroves(): Liquidating troves with SP deposits correctly impacts their SP deposit and AUT gain", async () => {
    ...
    // Check remaining LUSD Deposits and AUT gain, for whale and depositors whose troves were liquidated
  ...
  it("batchLiquidateTroves(): closes every trove with ICR < MCR in the given array", async () => {
    ...
    // Price drops to 1AUT:100LUSD, reducing A, B, C ICR below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice();
  ...
  it("redeemCollateral(): cancels the provided LUSD with debt from Troves with the lowest ICRs and sends an equivalent amount of Ether", async () => {
    ...
    const expectedTotalAUTDrawn = redemptionAmount.div(toBN(200)); // convert redemptionAmount LUSD to AUT, at AUT:USD price 200
  ...
  it("redeemCollateral(): ends the redemption sequence when the token redemption request has been filled", async () => {
    ...
    // open trove from redeemer.  Redeemer has highest ICR (100AUT, 100 LUSD), 20000%
  ...
  it("redeemCollateral(): doesnt perform the final partial redemption in the sequence if the hint is out-of-date", async () => {
    ...
    const expectedTotalAUTDrawn = fullfilledRedemptionAmount.sub(frontRunRedepmtion).div(toBN(200)); // redempted LUSD converted to AUT, at AUT:USD price 200
    const expectedReceivedAUT = expectedTotalAUTDrawn
      .sub(AUTFee)
      .sub(toBN(th.gasUsed(redemptionTx) * GAS_PRICE)); // substract gas used for troveManager.redeemCollateral from expected received AUT
  ...
  it.skip("redeemCollateral(): can redeem if there is zero active debt but non-zero debt in DefaultPool", async () => {
    ...
    const expectedTotalAUTDrawn = toBN(amount).div(toBN(100)); // convert 100 LUSD to AUT at AUT:USD price of 100
  ...
  it("redeemCollateral(): succeeds if fee is less than max fee percentage", async () => {
    ...
    const price = await priceFeed.getPrice();
    const AUTDrawn = attemptedLUSDRedemption.mul(mv._1e18BN).div(price);
  ...
  it("redeemCollateral(): doesn't affect the Stability Pool deposits or AUT gain of redeemed-from troves", async () => {
    ...
    // Check the remaining LUSD and AUT in Stability Pool after liquidation is non-zero
  ...
  it("redeemCollateral(): value of issued AUT == face value of redeemed LUSD (assuming 1 LUSD has value of $1)", async () => {
    ...
    /* 120 LUSD redeemed.  Expect $120 worth of AUT removed. At AUT:USD price of $200, 
    ...
    assert.equal(
      ...
      activeAUT_0.sub(toBN(_120_LUSD).mul(mv._1e18BN).div(price))
    );
    ...
    /* 373 LUSD redeemed.  Expect $373 worth of AUT removed. At AUT:USD price of $200, 
    ...
    assert.equal(
      ...
      activeAUT_1.sub(toBN(_373_LUSD).mul(mv._1e18BN).div(price))
    );
    ...
    /* 950 LUSD redeemed.  Expect $950 worth of AUT removed. At AUT:USD price of $200, 
    ...
    assert.equal(
      ...
      activeAUT_2.sub(toBN(_950_LUSD).mul(mv._1e18BN).div(price))
    );
  });
  ...
  it("redeemCollateral(): a redemption made at zero base rate send a non-zero AUTFee to LQTY staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 LQTY
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider);
    await lqtyToken.approve(lqtyStaking.address, dec(1, 18), { from: multisig });
    await lqtyStaking.stake(dec(1, 18), { from: multisig });
  ...
  it("redeemCollateral(): a redemption made at zero base increases the AUT-fees-per-LQTY-staked in LQTY Staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 LQTY
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider);
    await lqtyToken.approve(lqtyStaking.address, dec(1, 18), { from: multisig });
    await lqtyStaking.stake(dec(1, 18), { from: multisig });
    ...
    // Check LQTY Staking AUT-fees-per-LQTY-staked before is zero
    ...
    // Check LQTY Staking AUT-fees-per-LQTY-staked after is non-zero
    const F_AUT_After = await lqtyStaking.F_AUT();
    assert.isTrue(F_AUT_After.gt("0"));
  });

  it("redeemCollateral(): a redemption made at a non-zero base rate send a non-zero AUTFee to LQTY staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 LQTY
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider);
    await lqtyToken.approve(lqtyStaking.address, dec(1, 18), { from: multisig });
    await lqtyStaking.stake(dec(1, 18), { from: multisig });
  ...
  it("redeemCollateral(): a redemption made at a non-zero base rate increases AUT-per-LQTY-staked in the staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 LQTY
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider);
    await lqtyToken.approve(lqtyStaking.address, dec(1, 18), { from: multisig });
    await lqtyStaking.stake(dec(1, 18), { from: multisig });
  ...
  it("computeICR(): Returns 2^256-1 for AUT:USD = 100, coll = 1 AUT, debt = 100 LUSD", async () => {
    const price = dec(100, 18);
    const coll = dec(1, "ether");
    const debt = dec(100, 18);
  ...
  it("computeICR(): returns correct ICR for AUT:USD = 100, coll = 200 AUT, debt = 30 LUSD", async () => {
    const price = dec(100, 18);
  ...
  it("computeICR(): returns correct ICR for AUT:USD = 250, coll = 1350 AUT, debt = 127 LUSD", async () => {
  ...
  it("computeICR(): returns correct ICR for AUT:USD = 100, coll = 1 AUT, debt = 54321 LUSD", async () => {
    const price = dec(100, 18);
    const coll = dec(1, "ether");
