...
contract("StabilityPool - Withdrawal of stability deposit - Reward calculations", async accounts => {
  ...
  describe("Stability Pool Withdrawal", async () => {
    ...
    it("withdrawAUTGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and AUT Gain after one liquidation", async () => {
      ...
      // Check depositors' compounded deposit is 6666.66 LUSD and AUT Gain is 33.16 AUT
    ...
    it("withdrawAUTGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and AUT Gain after two identical liquidations", async () => {
      ...
      // Check depositors' compounded deposit is 3333.33 LUSD and AUT Gain is 66.33 AUT
      const txA = await stabilityPool.withdrawAUTGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, {
        from: alice
      });
      const txB = await stabilityPool.withdrawAUTGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, {
    ...
    it("withdrawAUTGainToTrove():  Depositors with equal initial deposit withdraw correct compounded deposit and AUT Gain after three identical liquidations", async () => {
      ...
      // Check depositors' compounded deposit is 0 LUSD and AUT Gain is 99.5 AUT
      const txA = await stabilityPool.withdrawAUTGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, {
        from: alice
      });
      const txB = await stabilityPool.withdrawAUTGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, {
    ...
    it("withdrawAUTGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and AUT Gain after two liquidations of increasing LUSD", async () => {
      // Whale opens Trove with 100k AUT
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveLUSDAmount(dec(100000, 18)),
    ...
    it("withdrawAUTGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and AUT Gain after three liquidations of increasing LUSD", async () => {
      // Whale opens Trove with 100k AUT
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveLUSDAmount(dec(100000, 18)),
    ...
    it("withdrawAUTGainToTrove(): Depositors with varying deposits withdraw correct compounded deposit and AUT Gain after three varying liquidations", async () => {
      ...
      Defaulter 1: 207000 LUSD & 2160 AUT
      Defaulter 2: 5000 LUSD & 50 AUT
      Defaulter 3: 46700 LUSD & 500 AUT
      */
      await borrowerOperations.openTrove(
        th._100pct,
    ...
    it("withdrawAUTGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 1 liquidation. All deposits and liquidations = 100 LUSD.  A, B, C, D withdraw correct LUSD deposit and AUT Gain", async () => {
      // Whale opens Trove with 100k AUT
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveLUSDAmount(dec(100000, 18)),
    ...
    it("withdrawAUTGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. All deposits and liquidations = 100 LUSD.  A, B, C, D withdraw correct LUSD deposit and AUT Gain", async () => {
      // Whale opens Trove with 100k AUT
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveLUSDAmount(dec(100000, 18)),
    ...
    it("withdrawAUTGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. Various deposit and liquidation vals.  A, B, C, D withdraw correct LUSD deposit and AUT Gain", async () => {
      // Whale opens Trove with 1m AUT
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveLUSDAmount(dec(1000000, 18)),
      ...
      */
      ...
      /* Defaulters open troves:
      Defaulter 1:  10000 LUSD, 100 AUT
      Defaulter 2:  25000 LUSD, 250 AUT
      Defaulter 3:  5000 LUSD, 50 AUT
      Defaulter 4:  40000 LUSD, 400 AUT
      */
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveLUSDAmount(dec(10000, 18)),
    ...
    it("withdrawAUTGainToTrove(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. All deposits and liquidations = 100 LUSD.  A, B, C, D withdraw correct LUSD deposit and AUT Gain", async () => {
      // Whale opens Trove with 100k AUT
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveLUSDAmount(dec(100000, 18)),
    ...
    it("withdrawAUTGainToTrove(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. Various deposit and liquidation vals. A, B, C, D withdraw correct LUSD deposit and AUT Gain", async () => {
      // Whale opens Trove with 100k AUT
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveLUSDAmount(dec(100000, 18)),
    ...
    it("withdrawAUTGainToTrove(): A, B, D deposit -> 2 liquidations -> C makes deposit -> 1 liquidation -> D withdraws -> 1 liquidation. All deposits: 100 LUSD. Liquidations: 100,100,100,50.  A, B, C, D withdraw correct LUSD deposit and AUT Gain", async () => {
      // Whale opens Trove with 100k AUT
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveLUSDAmount(dec(100000, 18)),
    ...
    // Expect all depositors withdraw 0 LUSD and 100 AUT
    ...
    it("withdrawAUTGainToTrove(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and AUT Gain after one liquidation", async () => {
      ...
      // Expect Bob to retain 1% of initial deposit (100 LUSD) and all the liquidated AUT (60 ether)
      assert.isAtMost(
        th.getDifference(
          (await stabilityPool.getCompoundedLUSDDeposit(bob)).toString(),
    ...
    it("withdrawAUTGainToTrove(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and AUT Gain after one liquidation", async () => {
      ...
      // Bob should withdraw 1e-5 of initial deposit: 0.1 LUSD and the full AUT gain of 100 ether
      assert.isAtMost(
    ...
    /* A make deposit 10000 LUSD
    L1 brings P to 0.0001P. L1:  9999.900000000000000000 LUSD, 1 AUT
    B makes deposit 9999.9, brings SP to 10k
    L2 decreases P by(~1e-5)P. L2:  9999.900000000000000000 LUSD, 1 AUT
    C makes deposit 9999.9, brings SP to 10k
    L3 decreases P by(~1e-5)P. L3:  9999.900000000000000000 LUSD, 1 AUT
    D makes deposit 9999.9, brings SP to 10k
    L4 decreases P by(~1e-5)P. L4:  9999.900000000000000000 LUSD, 1 AUT
    expect A, B, C, D each withdraw ~100 Ether
    */
    it("withdrawAUTGainToTrove(): Several deposits of 10000 LUSD span one scale factor change. Depositors withdraws correct compounded deposit and AUT Gain after one liquidation", async () => {
      // Whale opens Trove with 100k AUT
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveLUSDAmount(dec(100000, 18)),
    ...
    it("withdrawAUTGainToTrove(): Small liquidated coll/debt, large deposits and AUT price", async () => {
      ...
      // Defaulter opens trove with 50e-7 AUT and  5000 LUSD. 200% ICR
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveLUSDAmount(dec(5000, 18)),
        defaulter_1,
