...
class TestHelper {
  ...
  // stored in Liquity, or the current Chainlink AUTUSD price, etc.
  ...
  // Adds the gas compensation (50 ONEU)
  ...
  /*
   * given the desired total debt, returns the ONEU amount that needs to be requested in openTrove
   * So, it subtracts the gas compensation and then the borrowing fee
   */
  static async getOpenTroveONEUAmount(contracts, totalDebt) {
  ...
  static getEmittedRedemptionValues(redemptionTx) {
    for (let i = 0; i < redemptionTx.logs.length; i++) {
      if (redemptionTx.logs[i].event === "Redemption") {
        const ONEUAmount = redemptionTx.logs[i].args[0];
        const totalONEURedeemed = redemptionTx.logs[i].args[1];
        const totalAUTDrawn = redemptionTx.logs[i].args[2];
        const AUTFee = redemptionTx.logs[i].args[3];

        return [ONEUAmount, totalONEURedeemed, totalAUTDrawn, AUTFee];
      }
    }
  ...
  static getONEUFeeFromONEUBorrowingEvent(tx) {
    for (let i = 0; i < tx.logs.length; i++) {
      if (tx.logs[i].event === "ONEUBorrowingFeePaid") {
        return tx.logs[i].args[1].toString();
      }
    }
    throw "The transaction logs do not contain an ONEUBorrowingFeePaid event";
  }
  ...
  static async getEntireCollAndDebt(contracts, account) {
    ...
    const pendingAUTReward = await contracts.troveManager.getPendingAUTReward(account);
    const pendingONEUDebtReward = await contracts.troveManager.getPendingONEUDebtReward(account);
    const entireColl = rawColl.add(pendingAUTReward);
    const entireDebt = rawDebt.add(pendingONEUDebtReward);
  ...
  static async getCollAndDebtFromWithdrawONEU(contracts, account, amount) {
  ...
  static async getCollAndDebtFromRepayONEU(contracts, account, amount) {
    const { entireColl, entireDebt } = await this.getEntireCollAndDebt(contracts, account);
  ...
  static async getCollAndDebtFromAdjustment(contracts, account, AUTChange, ONEUChange) {
    const { entireColl, entireDebt } = await this.getEntireCollAndDebt(contracts, account);

    // const coll = (await contracts.troveManager.Troves(account))[1]
    // const debt = (await contracts.troveManager.Troves(account))[0]

    const fee = ONEUChange.gt(this.toBN("0"))
      ? await contracts.troveManager.getBorrowingFee(ONEUChange)
      : this.toBN("0");
    const newColl = entireColl.add(AUTChange);
    const newDebt = entireDebt.add(ONEUChange).add(fee);
  ...
  static async openTrove_allAccounts(accounts, contracts, AUTAmount, ONEUAmount) {
    const gasCostList = [];
  ...
  static async openTrove_allAccounts_randomAUT(minAUT, maxAUT, accounts, contracts, ONEUAmount) {
    const gasCostList = [];
    const totalDebt = await this.getOpenTroveTotalDebt(contracts, ONEUAmount);

    for (const account of accounts) {
      const randCollAmount = this.randAmountInWei(minAUT, maxAUT);
      const { upperHint, lowerHint } = await this.getBorrowerOpsListHint(
        contracts,
        randCollAmount,
        totalDebt
  ...
  static async openTrove_allAccounts_randomAUT_ProportionalONEU(
    minAUT,
    maxAUT,
    accounts,
    contracts,
    ...
    for (const account of accounts) {
      const randCollAmount = this.randAmountInWei(minAUT, maxAUT);
      const proportionalONEU = web3.utils.toBN(proportion).mul(web3.utils.toBN(randCollAmount));
      const totalDebt = await this.getOpenTroveTotalDebt(contracts, proportionalONEU);
  ...
  static async openTrove_allAccounts_randomAUT_randomONEU(
    minAUT,
    maxAUT,
    accounts,
    contracts,
    ...
    for (const account of accounts) {
      ...
      const randONEUProportion = this.randAmountInWei(minONEUProportion, maxONEUProportion);
      const proportionalONEU = web3.utils
        .toBN(randONEUProportion)
      ...
      if (logging && tx.receipt.status) {
        i++;
        const ICR = await contracts.troveManager.getCurrentICR(account, price);
        // console.log(`${i}. Trove opened. addr: ${this.squeezeAddr(account)} coll: ${randCollAmount} debt: ${proportionalONEU} ICR: ${ICR}`)
      }
      const gas = this.gasUsed(tx);
      gasCostList.push(gas);
    }
  ...
  static async openTrove_allAccounts_randomONEU(minONEU, maxONEU, accounts, contracts, AUTAmount) {
    const gasCostList = [];

    for (const account of accounts) {
      const randONEUAmount = this.randAmountInWei(minONEU, maxONEU);
      const totalDebt = await this.getOpenTroveTotalDebt(contracts, randONEUAmount);
      const { upperHint, lowerHint } = await this.getBorrowerOpsListHint(
        contracts,
        AUTAmount,
        totalDebt
  ...
  static async openTrove_allAccounts_decreasingONEUAmounts(
    accounts,
    contracts,
    AUTAmount,
    maxONEUAmount
    ...
    let i = 0;
    for (const account of accounts) {
      const ONEUAmount = (maxONEUAmount - i).toString();
      const ONEUAmountWei = web3.utils.toWei(ONEUAmount, "ether");
      const totalDebt = await this.getOpenTroveTotalDebt(contracts, ONEUAmountWei);
      const { upperHint, lowerHint } = await this.getBorrowerOpsListHint(
        contracts,
        AUTAmount,
        totalDebt
  ...
  static async openTrove(
    contracts,
    { maxFeePercentage, extraONEUAmount, upperHint, lowerHint, ICR, extraParams }
  ) {
    if (!maxFeePercentage) maxFeePercentage = this._100pct;
    if (!extraONEUAmount) extraONEUAmount = this.toBN(0);
    else if (typeof extraONEUAmount == "string") extraONEUAmount = this.toBN(extraONEUAmount);
  ...
  static async adjustTrove_allAccounts(accounts, contracts, AUTAmount, ONEUAmount) {
    const gasCostList = [];

    for (const account of accounts) {
      let tx;

      let AUTChangeBN = this.toBN(AUTAmount);
      let ONEUChangeBN = this.toBN(ONEUAmount);
      ...
      let isDebtIncrease = ONEUChangeBN.gt(zero);
      ONEUChangeBN = ONEUChangeBN.abs();

      // Add AUT to trove
      if (AUTChangeBN.gt(zero)) {
        tx = await contracts.borrowerOperations.adjustTrove(
          ...
          upperHint,
          lowerHint,
          { from: account, value: AUTChangeBN }
        );
        // Withdraw AUT from trove
      } else if (AUTChangeBN.lt(zero)) {
        AUTChangeBN = AUTChangeBN.neg();
        tx = await contracts.borrowerOperations.adjustTrove(
          this._100pct,
          AUTChangeBN,
          ONEUChangeBN,
  ...
  static async adjustTrove_allAccounts_randomAmount(
    ...
    for (const account of accounts) {
      ...
      let AUTChangeBN = this.toBN(this.randAmountInWei(AUTMin, AUTMax));
      let ONEUChangeBN = this.toBN(this.randAmountInWei(ONEUMin, ONEUMax));
      ...
      const gas = this.gasUsed(tx);
      // console.log(`AUT change: ${AUTChangeBN},  ONEUChange: ${ONEUChangeBN}, gas: ${gas} `)
  ...
  static async withdrawONEU_allAccounts(accounts, contracts, amount) {
    const gasCostList = [];

    for (const account of accounts) {
      const { newColl, newDebt } = await this.getCollAndDebtFromWithdrawONEU(
        contracts,
        account,
        amount
      );
      ...
      const tx = await contracts.borrowerOperations.withdrawONEU(
        this._100pct,
        amount,
        upperHint,
        lowerHint,
  ...
  static async withdrawONEU_allAccounts_randomAmount(min, max, accounts, contracts) {
    const gasCostList = [];

    for (const account of accounts) {
      const randONEUAmount = this.randAmountInWei(min, max);
  ...
  static async repayONEU_allAccounts(accounts, contracts, amount) {
    const gasCostList = [];

    for (const account of accounts) {
      const { newColl, newDebt } = await this.getCollAndDebtFromRepayONEU(
        contracts,
        account,
        amount
      );
      ...
      const tx = await contracts.borrowerOperations.repayONEU(amount, upperHint, lowerHint, {
        from: account
      });
      const gas = this.gasUsed(tx);
      gasCostList.push(gas);
  ...
  static async repayONEU_allAccounts_randomAmount(min, max, accounts, contracts) {
    const gasCostList = [];
    ...
    for (const account of accounts) {
      ...
      const tx = await contracts.borrowerOperations.repayONEU(randONEUAmount, upperHint, lowerHint, {
        from: account
      });
      const gas = this.gasUsed(tx);
      gasCostList.push(gas);
  ...
  static async redeemCollateral_allAccounts_randomAmount(min, max, accounts, contracts) {
    ...
    for (const redeemer of accounts) {
      ...
      await this.performRedemptionTx(redeemer, price, contracts, randONEUAmount);
      const gas = this.gasUsed(tx);
      gasCostList.push(gas);
    }
    return this.getGasMetrics(gasCostList);
  ...
  static async provideToSP_allAccounts_randomAmount(min, max, accounts, stabilityPool) {
    const gasCostList = [];
    for (const account of accounts) {
      const randomONEUAmount = this.randAmountInWei(min, max);
      const tx = await stabilityPool.provideToSP(randomONEUAmount, this.ZERO_ADDRESS, {
        from: account
      });
      const gas = this.gasUsed(tx);
      gasCostList.push(gas);
  ...
  static async withdrawFromSP_allAccounts_randomAmount(min, max, accounts, stabilityPool) {
    ...
    for (const account of accounts) {
      const randomONEUAmount = this.randAmountInWei(min, max);
      const tx = await stabilityPool.withdrawFromSP(randomONEUAmount, { from: account });
      const gas = this.gasUsed(tx);
      gasCostList.push(gas);
    }
    return this.getGasMetrics(gasCostList);
  ...
  static async withdrawAUTGainToTrove_allAccounts(accounts, contracts) {
    const gasCostList = [];
    for (const account of accounts) {
      let { entireColl, entireDebt } = await this.getEntireCollAndDebt(contracts, account);
      console.log(`entireColl: ${entireColl}`);
      console.log(`entireDebt: ${entireDebt}`);
      const AUTGain = await contracts.stabilityPool.getDepositorAUTGain(account);
      const newColl = entireColl.add(AUTGain);
      const { upperHint, lowerHint } = await this.getBorrowerOpsListHint(
        contracts,
        newColl,
        entireDebt
      ...
      const tx = await contracts.stabilityPool.withdrawAUTGainToTrove(upperHint, lowerHint, {
        from: account
      });
      const gas = this.gasUsed(tx);
      gasCostList.push(gas);
