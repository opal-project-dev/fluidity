...
async function main() {
  ...
  const OPLContracts = await dh.deployOPLContractsHardhat(
    ARBITRARY_ADDRESS,
    ARBITRARY_ADDRESS,
    ARBITRARY_ADDRESS
  );
  ...
  await dh.connectCoreContracts(coreContracts, OPLContracts);
  await dh.connectOPLContracts(OPLContracts);
  await dh.connectOPLContractsToCore(OPLContracts, coreContracts);
  ...
  const ONEUAmount = toBN(toWei("2500")); // borrower wants to withdraw 2500 ONEU
  const AUTColl = toBN(toWei("5")); // borrower wants to lock 5 AUT collateral

  // Call deployed TroveManager contract to read the liquidation reserve and latest borrowing fee
  const liquidationReserve = await troveManager.ONEU_GAS_COMPENSATION();
  const expectedFee = await troveManager.getBorrowingFeeWithDecay(ONEUAmount);

  // Total debt of the new trove = ONEU amount drawn, plus fee, plus the liquidation reserve
  const expectedDebt = ONEUAmount.add(expectedFee).add(liquidationReserve);

  // Get the nominal NICR of the new trove
  const _1e20 = toBN(toWei("100"));
  let NICR = AUTColl.mul(_1e20).div(expectedDebt);
  ...
  await borrowerOperations.openTrove(maxFee, ONEUAmount, upperHint, lowerHint, { value: AUTColl });

  // --- adjust trove ---

  const collIncrease = toBN(toWei("1")); // borrower wants to add 1 AUT
  const ONEURepayment = toBN(toWei("230")); // borrower wants to repay 230 ONEU

  // Get trove's current debt and coll
  const { 0: debt, 1: coll } = await troveManager.getEntireDebtAndColl(borrower);

  const newDebt = debt.sub(ONEURepayment);
  ...
  await borrowerOperations.adjustTrove(maxFee, 0, ONEURepayment, false, upperHint, lowerHint, {
    value: collIncrease
  });
  ...
  const redemptionhint = await hintHelpers.getRedemptionHints(ONEUAmount, price, 50);

  const {
    0: firstRedemptionHint,
    1: partialRedemptionNewICR,
    2: truncatedONEUAmount
  } = redemptionhint;
  ...
  /* Finally, perform the on-chain redemption, passing the truncated ONEU amount, the correct hints, and the expected
