...
import { EthersLiquity, ReadableEthersLiquity } from "@fluidity/lib-ethers";
import { SubgraphLiquity } from "@fluidity/lib-subgraph";
...
export const createRandomTrove = (price: Decimal) => {
  ...
  if (Math.random() < 0.5) {
    ...
    const debt = ONEU_LIQUIDATION_RESERVE.add(truncateLastDigits(maxDebt - benford(maxDebt)));

    return new Trove(collateral, debt);
  } else {
    const debt = ONEU_LIQUIDATION_RESERVE.add(100 * randomValue);
...
export const randomDebtChange = ({ debt }: Trove) =>
  Math.random() < 0.5
    ? { repayONEU: debt.mul(1.1 * Math.random()) }
    : { borrowONEU: debt.mul(0.5 * Math.random()) };
...
const checks = [
  ...
  new EqualityCheck("tokensInStabilityPool", l => l.getONEUInStabilityPool(), decimalsEqual)
];
