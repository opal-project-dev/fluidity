...
import { LiquityStoreState, Decimal, Trove, Decimalish, ONEU_MINIMUM_DEBT } from "@fluidity/lib-base";
...
const reduce = (state: TroveManagerState, action: TroveManagerAction): TroveManagerState => {
  ...
  switch (action.type) {
    ...
    case "addMinimumDebt":
      return {
        ...state,
        edited: edited.setDebt(ONEU_MINIMUM_DEBT),
...
const feeFrom = (original: Trove, edited: Trove, borrowingRate: Decimal): Decimal => {
  ...
  if (change && change.type !== "invalidCreation" && change.params.borrowONEU) {
    return change.params.borrowONEU.mul(borrowingRate);
  } else {
    return Decimal.ZERO;
  }
};
  ...
  return (
    ...
    >
      {description ??
        (openingNewTrove ? (
          <ActionDescription>
            Start by entering the amount of NTN you'd like to deposit as collateral.
          </ActionDescription>
        ) : (
          <ActionDescription>
